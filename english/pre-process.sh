...
lines=$(wc -l < english_pets_cleaned.csv)
echo "There are $lines lines in the dataset!"

# Check for empty lines
empty_lines=$(grep -c '^$' english_pets_cleaned.csv)
...
columns=$(awk -F ',' '{print NF; exit}' english_pets_cleaned.csv)
echo "There are $columns columns in the dataset."

# Normalize case and remove punctuation
echo "Normalizing case and removing punctuation."
tr '[:upper:]' '[:lower:]' < english_pets_cleaned.csv | tr -d '[:punct:]' > normalized_texts.txt
